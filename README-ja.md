...
# 2. リマッパーを起動
midimap -i 0 -o 1 -s config.lua

# 3. verboseモードで起動
midimap -i 0 -o 1 -s config.lua -v
```
...
```
midimap -l
midimap -i <入力> -o <出力> [-s config.lua] [-v]
```
...
| `-s`, `-script <ファイル>` | Luaリマップスクリプト(デフォルト: `config.lua`) |
